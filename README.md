...
Kioo brings Enlive/Enfocus style templates to React.  This allows for much better separation between the view and the logic of the application.  

### Why does kioo mean? 
Kioo is swahili for mirror. Reactjs is built around the idea that the view is a reflection of your application state.
...
(defn my-nav-item [[caption func]]
  (kioo/component "templates/main.html" [:.nav-item]
    {[:a] (do-> (content caption)
                (set-attr :onClick func))}))


(defn my-header [heading nav-elms]
  (kioo/component "templates/main.html" [:header :>]
    {[:h1] (content heading)
     [ul]  (content (map my-nav-item nav-elements))}))

(defn my-page [data]
  (kioo/component "templates/main"
    {[:header] (content (my-header (:heading data) (:nav-elms data)))
...
(om/root app-state my-page js/document.body)
...
Kioo support most of the enlive transform functions:
